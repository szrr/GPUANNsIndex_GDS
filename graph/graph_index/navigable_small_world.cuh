...
public:
    ...
    void SearchTopKonDevice(float* d_queries, int num_of_topk, int* &results, int num_of_query_points, int num_of_candidates, int* d_enter_cluster, GPUIndex* d_rvq_index, Timer* &graphSearch, int search_width){
    	...
    	NSWGraphOperations::Search(d_data, d_queries, graph_, results, num_of_query_points, points_->GetNumPoints(), 
                                   ...
                                   d_enter_cluster, d_rvq_index, graphSearch, search_width);
    ...
    void Establishment(int num_of_initial_neighbors, int num_of_candidates){
        ...
        distance_ = new float[points_->GetNumPoints() << offset_shift_];
        ...
        NSWGraphOperations::LocalGraphMergenceCoorperativeGroup(d_points, graph_, points_->GetNumPoints(), points_->GetDimofPoints(), offset_shift_, num_of_initial_neighbors_, num_of_batches_, 
                                                                    num_of_points_one_batch_, d_neighbors, d_neighbors_backup, num_of_maximal_neighbors_, num_of_candidates, first_subgraph, distance_);
    ...
    void Dump(string graph_name, string dis_name, string final_graph_name, int num_of_points, int total_num_of_points){
        for(int i = 0; i < points_->GetNumPoints(); i++){
            idx_t *loc = graph_ + i * (1 << offset_shift_);
            for(int l = 0; l < (1 << offset_shift_); l++){
        ...
        out_graph.write((char*)graph_, sizeof(int) * (points_->GetNumPoints() << offset_shift_));
        out_graph.close();

        ofstream out_dis(dis_name, std::ios::binary);
        out_dis.write((char*)distance_, sizeof(float) * (points_->GetNumPoints() << offset_shift_));
        out_dis.close();
