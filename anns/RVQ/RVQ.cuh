...
public:
    ...
    RVQ(int dim, int numCoarseCentroids, int numFineCentroids)
        ...
        clusterSize = new int[numCoarseCentroids * numFineCentroids];
    ...
    // 训练粗略量化码本
    void train(std::string base_path, idx_t numVectors);
    ...
    void copyIndexToGPU(GPUIndex* d_index);
    void copyIndexToGPU(GPUIndex* d_index, int num_of_query, int* cluster_of_query, int dim);
    void get_warmup_vectors(float *&d_warpup_vectors, int &num_of_warmup);
