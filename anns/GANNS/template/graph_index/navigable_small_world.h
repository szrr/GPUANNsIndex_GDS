...
public:
    ...
    void SearchTopKonDevice(float* queries, int num_of_topk, int* &results, int num_of_query_points, int num_of_candidates, int* groundtruth){
    	...
    	NSWGraphOperations::Search(points_->GetFirstPositionofPoint(0), queries, graph_, results, num_of_query_points, points_->GetNumPoints(), points_->GetDimofPoints(), offset_shift_, num_of_topk_, num_of_candidates, num_of_explored_points, groundtruth);
    ...
    void Establishment(int num_of_initial_neighbors, int num_of_candidates){
        ...
        /*for (int i = 0; i < num_of_points_one_batch_; i++) {
            for(int l = 0;l < num_of_maximal_neighbors_;l++){
                printf("%d ",first_subgraph[i * num_of_maximal_neighbors_ + l].second);
            }
            printf("\n");
        }*/
