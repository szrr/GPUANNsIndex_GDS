...
__global__
void DistanceOfEntryPoints(float* d_data, float* d_query,KernelPair<float, int>* d_entry_points,int entry_points_num,int entry_points_num_sort){
    int b_id=blockIdx.x;
    ...
    for(entry_point_id = 0; entry_point_id < entry_points_num; entry_point_id++){
        int target_point_id = d_entry_points[crt_point_id * entry_points_num_sort + entry_point_id].second;
    ...
    if (t_id == 0) {
        ...
        if (t_id == 0) {
            //printf("%f \n",dist);
            d_entry_points[crt_point_id * entry_points_num_sort + entry_point_id].first = dist;
        }           
    }
    ...
    int step_id = 1;
    int substep_id = 1;
    int tmp_num = crt_point_id * entry_points_num_sort;
    KernelPair<float,int> temporary_neighbor;
    for (; step_id <= entry_points_num_sort / 2; step_id *= 2) {
        substep_id = step_id;

        for (; substep_id >= 1; substep_id /= 2) {
            for (int temparory_id = 0; temparory_id < (entry_points_num_sort/2+size_of_warp-1) / size_of_warp; temparory_id++) {
                ...
                if (unrollt_id < tmp_num + entry_points_num_sort) {
...
void SearchDevice(float* d_data, float* d_query, int* d_result, int* d_graph, int total_num_of_points, int num_of_query_points, int offset_shift, 
                    ...
                    KernelPair<float, int>* d_entry_points, int num_of_entry_points, int num_of_entry_points_sort) {
	int t_id = threadIdx.x;
    int b_id = blockIdx.x;
    int size_of_warp = 32;
    ...
    for (int i = 0; i < (num_of_candidates + num_of_visited_points_one_batch + size_of_warp - 1) / size_of_warp; i++) {
        ...
        if (unrollt_id < num_of_candidates + num_of_visited_points_one_batch) {
            ...
            if(unrollt_id < num_of_candidates && unrollt_id < num_of_entry_points){
                neighbors_array[unrollt_id].first = d_entry_points[(crt_point_id + 1) * num_of_entry_points_sort - unrollt_id - 1].first;
                neighbors_array[unrollt_id].second = d_entry_points[(crt_point_id + 1) * num_of_entry_points_sort - unrollt_id - 1].second;
            }
            else{
                neighbors_array[unrollt_id].first = Max;
                neighbors_array[unrollt_id].second = total_num_of_points;
    ...
    //int target_point_id = neighbors_array[0].second;
