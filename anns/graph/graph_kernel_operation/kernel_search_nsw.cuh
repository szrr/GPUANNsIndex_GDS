...
void SearchDevice(float* d_data, float* d_query, int* d_result, int* d_graph, int total_num_of_points, int num_of_query_points, int offset_shift, 
                    ...
                    int* d_enter_cluster, int** d_rvq_indices, int* d_rvq_indices_size, int shared_mem_size, int* d_count) {
	int t_id = threadIdx.x;
    int b_id = blockIdx.x;
    int size_of_warp = 32;
...
//Search    
    ...
    if(enter_points_num == 0){
        if(t_id == 0){
            neighbors_array[num_of_candidates].second = 0;
            flags[num_of_candidates] = 1;
        }
    }else{
        ...
        for (int i = 0; i < (num_of_visited_points_one_batch + size_of_warp - 1) / size_of_warp; i++) {
            ...
            if (unrollt_id < num_of_visited_points_one_batch && unrollt_id < enter_points_num) { //unrollt_id < enter_points_num
                
                neighbors_array[num_of_candidates + unrollt_id].second = enter_points_pos[unrollt_id];
