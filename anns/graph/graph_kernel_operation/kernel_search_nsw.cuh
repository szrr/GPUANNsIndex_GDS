...
__global__
void DistanceOfEntryPoints(float* d_data, float* d_query,KernelPair<float, int>* d_entry_points,int* enter_points_num,int* enter_points_num_sort){
    int b_id=blockIdx.x;
...
#endif
    int num = enter_points_num[crt_point_id];
    for(enter_point_id = 0; enter_point_id < num; enter_point_id++){
        int target_point_id = d_entry_points[enter_points_num_sort[crt_point_id] + enter_point_id].second;
    ...
    if (t_id == 0) {
        ...
        if (t_id == 0) {
            //printf("%f \n",dist);
            d_entry_points[enter_points_num_sort[crt_point_id] + enter_point_id].first = dist;
        }           
    }
    ...
    int step_id = 1;
    int substep_id = 1;
    int tmp_num = enter_points_num_sort[crt_point_id];
    int len = enter_points_num_sort[crt_point_id + 1] - enter_points_num_sort[crt_point_id];
    KernelPair<float,int> temporary_neighbor;
    for (; step_id <= len / 2; step_id *= 2) {
        substep_id = step_id;

        for (; substep_id >= 1; substep_id /= 2) {
            for (int temparory_id = 0; temparory_id < (len/2+size_of_warp-1) / size_of_warp; temparory_id++) {
                int unrollt_id = tmp_num + ((t_id + size_of_warp * temparory_id) / substep_id) * 2 * substep_id + ((t_id + size_of_warp * temparory_id) & (substep_id - 1));
                
                if (unrollt_id < tmp_num + len) {
                    if (((t_id + size_of_warp * temparory_id) / step_id) % 2 == 0) {
                        if (d_entry_points[unrollt_id].first < d_entry_points[unrollt_id + substep_id].first) {
                            temporary_neighbor = d_entry_points[unrollt_id];
                            d_entry_points[unrollt_id] = d_entry_points[unrollt_id + substep_id];
                            d_entry_points[unrollt_id + substep_id] = temporary_neighbor;
                        }
                    } else {
                        if (d_entry_points[unrollt_id].first > d_entry_points[unrollt_id + substep_id].first) {
                            temporary_neighbor = d_entry_points[unrollt_id];
...
void SearchDevice(float* d_data, float* d_query, int* d_result, int* d_graph, int total_num_of_points, int num_of_query_points, int offset_shift, 
                    ...
                    int* d_entry_points, int* num_of_entry_points, int shared_mem_size, int *d_count) {
	int t_id = threadIdx.x;
    int b_id = blockIdx.x;
    int size_of_warp = 32;
...
//Search    
    ...
    if (t_id == 0) {
        ...
        int enter_points_num = num_of_entry_points[crt_point_id + 1] - num_of_entry_points[crt_point_id];
        int enter_points_pos = num_of_entry_points[crt_point_id];
        auto stage2_start = clock64();
        for (int i = 0; i < (num_of_visited_points_one_batch + size_of_warp - 1) / size_of_warp; i++) {
            int unrollt_id = t_id + size_of_warp * i;

            if (unrollt_id < num_of_visited_points_one_batch && unrollt_id < enter_points_num) {
                neighbors_array[num_of_candidates + unrollt_id].second = (d_entry_points + enter_points_pos)[unrollt_id];
