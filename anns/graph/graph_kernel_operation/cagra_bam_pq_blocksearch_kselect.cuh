...
__global__ void cagra_bam_pq_blocksearch_kselect(array_d_t<char>* d_data, float* d_query, int* d_result, int total_num_of_points, int offset_shift, 
    ...
    int num_of_visited_points_one_batch = (1 << offset_shift) * num_elements_per_block;
    ...
    KernelPair<float, int>* final_neighbors_array = shared_memory_space_s;
...
//Search    
    ...
    for (int i = warp_id; i < enter_points_num; i += warp_size) {
        size_t target_point_id = size_t(enter_points_pos[i]);
    ...
    while (flag_all_blocks /*&& iter < max_iter*/){
        ...
        while(tmp_search_width < search_width && tmp_flag != 0){
            ...
            first_position_of_flag = d_starling_index[abs(first_position_of_flag)];
            ...
            if(!tmp_test(int(block_id), random_number, block_data)){
                ...
                if(num_of_read + num_of_points_in_block >= num_of_points_to_sort){
                    auto sort_start = clock64();
                    step_id = 1;
                    substep_id = 1;
                ...
                for(size_t i = 0; i < num_of_points_in_block; i++){
                    ...
                    for (int l = 0; l < tmp_degree_of_point; l++) {
    
                        int target_point_id = (*neighbor_data)[offset + i * size_of_element + DIM + size_t(l + 1)]; 
                        ...
                        dist = __shfl_sync(0xFFFFFFFF, dist, 0);
                        if (lane_id == (l + i * degree_of_point + tmp_search_width * num_of_visited_points_one_batch) % size_of_warp) {
                            heap.addThreadQ(dist, target_point_id);
                            add(target_point_id, random_number, data);
                        }
                    }
                    ...
                    if (lane_id == 0) {
                        ...
                        final_neighbors_array[final_num_of_candidates + num_of_read].second = first_position_of_flag + i;
                        // if(b_id == 0){
