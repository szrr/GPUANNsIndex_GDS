...
void SortNeighborsonLocalGraph(KernelPair<float, int>* neighbors, KernelPair<float, int>* old_neighbors, int total_num_of_points, float* d_data, 
                                    int num_of_points_one_batch, int length_of_sequence, int offset_shift, KernelPair<float, int>* distance_matrix, int batches, int num_of_batches_tmp){
    ...
    int num_of_iterations = (num_of_batches_tmp * num_of_points_one_batch + b_dim - 1) / b_dim;
    int step_id = 1;
    int substep_id = 1;
    int size_of_warp = 32;
    ...
    for (int i = 0; i < num_of_iterations; i++) {
        ...
        int crt_point_id = batches * num_of_batches_tmp * num_of_points_one_batch + i * b_dim + b_id;
