...
__global__ void cagra_bam_pq(array_d_t<char>* d_data, float* d_query, int* d_result, int total_num_of_points, int offset_shift, 
                    ...
                    int *d_enter_cluster, int** d_rvq_indices, int* d_rvq_indices_size, size_t hash_size, uint32_t* d_hash_table, 
                    size_t page_size, size_t num_elements_per_block, unsigned long long* time_breakdown, size_t* IO_count, float *d_dist_vec, 
                    uint8_t *d_pq_data, size_t num_of_chunks, int *d_starling_index, int *d_cache_result, int num_of_cache_candidates, int *num_of_block_accesses){
...
//Search    
    ...
    // int enter_points_num = num_of_cache_candidates;
    ...
    iteration = 0;
    int iter = 0;
    int max_iter = 1.5 * num_of_explored_points;
    while (flag_all_blocks /*&& iter < max_iter*/){
        ...
        while(tmp_search_width < search_width && tmp_flag != 0){
            ...
            size_t offset = (size_t(first_position_of_flag) / num_elements_per_block) * page_size / sizeof(int) + 
                            (size_t(first_position_of_flag) % num_elements_per_block) * (DIM + size_t(degree_of_point + 1));
            //if(t_id < size_of_warp){
                auto load_neighbors_start = clock64();
                //读取邻居
                int tmp_degree_of_point = (*neighbor_data)[offset + DIM];
                for (int i = 0; i < (degree_of_point + size_of_warp - 1) / size_of_warp; i++) {
                    ...
                    if (unrollt_id < degree_of_point) {
                        ...
                        if(unrollt_id >= tmp_degree_of_point){
                            ...
                            int target_point = (*neighbor_data)[offset + DIM + size_t(unrollt_id + 1)]; 
                            // int target_point = d_starling_index[(*neighbor_data)[offset + DIM + size_t(unrollt_id + 1)]]; 
            ...
            //}else{
                ...
                if (lane_id == 0) {
                    final_neighbors_array[final_num_of_candidates + num_of_read].first = dist; 
                    final_neighbors_array[final_num_of_candidates + num_of_read].second = abs(neighbors_array[neighbor_loc].second);
                    // if(b_id == 0){
            ...
            num_of_read++;
            __syncthreads();
            if(num_of_read == num_of_points_to_sort){
                auto sort_start = clock64();
                step_id = 1;
                substep_id = 1;
                ...
                // if(b_id == 0 && t_id == 0){
                //     for(int i = 0; i < final_num_of_candidates; i++){
                //         printf("Points: %d, Dist: %f\n", final_neighbors_array[i].second, final_neighbors_array[i].first);
                //     }
                //     printf("\n");
                // }
                num_of_read = 0;
