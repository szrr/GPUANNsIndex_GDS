...
__global__ void cagra_bam_pq_top1blocksearch(array_d_t<char>* d_data, float* d_query, int* d_result, int total_num_of_points, int offset_shift, 
    ...
    while (flag_all_blocks /*&& iter < max_iter*/){
        ...
        while(tmp_search_width < search_width && tmp_flag != 0){
            ...
            if(!tmp_test(int(block_id), random_number, block_data)){
                ...
                min_dis = __shfl_sync(0xFFFFFFFF, min_dis, 0);
                min_id_in_block = __shfl_sync(0xFFFFFFFF, min_id_in_block, 0);

                //读取邻居
                auto load_neighbors_start = clock64();
                int tmp_degree_of_point = (*neighbor_data)[offset + min_id_in_block * size_of_element + DIM];
                for (int l = 0; l < (tmp_degree_of_point + size_of_warp - 1) / size_of_warp; l++) {
                    ...
                    if (unrollt_id < tmp_degree_of_point) {
                        ...
                        int target_point = (*neighbor_data)[offset + min_id_in_block * size_of_element + DIM + size_t(unrollt_id + 1)];  
