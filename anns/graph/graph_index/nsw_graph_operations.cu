...
void NSWGraphOperations::Search(float* d_data, float* d_query, int* h_graph, int* h_result, int num_of_query_points, int total_num_of_points, int dim_of_point, 
	...
	readCagraGraph("/home/szr/test/cagra_test/sift10m_index.bin", total_num_of_points, 1 << offset_shift, h_graph);
	int* d_graph;
	cudaMalloc(&d_graph, sizeof(int) * (total_num_of_points << offset_shift));
	...
	graphSearch[2].Start();
	// SearchDevice<<<num_of_query_points, 32, (T_size + num_of_candidates) * (sizeof(KernelPair<float, int>))>>>
	...
	// 												d_enter_cluster, d_rvq_index->indices, d_rvq_index->sizes, T_size, 
	// 												d_count, d_zero_count, d_iter, epsilon);																													
	// cudaDeviceSynchronize();
	constexpr int WARP_SIZE = 32;
	constexpr int NumWarpQ = 64;
	constexpr int NumThreadQ = 2;
	
	cagra<int, float, WARP_SIZE, NumWarpQ, NumThreadQ><<<num_of_query_points, 32>>> //32, num_of_candidates * (sizeof(KernelPair<float, int>)
													(d_data, d_query, d_result, d_graph, total_num_of_points, num_of_query_points, 
													offset_shift, num_of_candidates, num_of_topk, num_of_explored_points, d_time_breakdown, search_width,  
													epsilon, d_enter_cluster, d_rvq_index->indices, d_rvq_index->sizes, T_size, d_count, d_iter, d_blockTimes);
	// testSearch<<<num_of_query_points, 32, (T_size + num_of_candidates) * (sizeof(KernelPair<float, int>))>>>
	// 												(d_data, d_query, d_result, d_graph, total_num_of_points, num_of_query_points, 
	// 												offset_shift, num_of_candidates, num_of_topk, num_of_explored_points, d_time_breakdown, search_width,  
	// 												epsilon, d_enter_cluster, d_rvq_index->indices, d_rvq_index->sizes, T_size, d_count, d_iter,
	// 												d_new_index_of_data, d_old_index_of_data);
	cudaDeviceSynchronize();
	error_check(cudaGetLastError(), __LINE__);
	graphSearch[2].Stop();
