...
public:
    ...
    void hybrid_build(const std::string& filename, int num_of_points){
        ...
        num_of_subgraph_points.resize(rvq->get_numFineCentroid());
        for (auto& inner : num_of_subgraph_points){
            inner.resize(rvq->get_numCoarseCentroid());
        }

        vector<vector<int>> new_index_of_points;
        new_index_of_points.resize(rvq->get_numFineCentroid());
        ...
        for(int iter = 0; iter < iteration; iter++){
            ...
            //build
            printf("Add %d points in RVQ\n", tmp_num_of_batch_points);
            rvq->build(d_sub_data, tmp_num_of_batch_points);
            // printf("Save subgraph data\n");
            // rvq->saveSubgraphData(sub_data_char, new_index_of_points,num_of_subgraph_points, num_of_batch_points, iter);
            cudaFree(d_sub_data);
            delete[] sub_data;
            delete[] sub_data_char;
        }
        in.close();
        //rvq->saveSubgraphIndex(num_of_points, new_index_of_points);
    }
    ...
    void hybrid_search(float* queries, int topk, int* &results, int num_queries, int num_candidates, int search_width){
