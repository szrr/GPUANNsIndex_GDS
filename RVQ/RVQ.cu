...
// 构建反向索引
void RVQ::build(float* d_buildVectorData, num_t num) {
    ...
    for(int iter = 0; iter < iteration; iter++){
        ...
        for(idx_t i = 0; i < tmpNumVectors; ++i){
            index_[minCoarseIndices[i]][minFineIndices[i]].push_back(i + numVectors * iter);
...
void RVQ::saveSubgraphData(char* sub_data, std::vector<std::vector<int>> &new_index_of_points, std::vector<std::vector<int>> &num_of_subgraph_points, idx_t numBatchPoints, int outIteration){
    for(int i = 0; i < index_.size(); i++){
        std::ostringstream filename;
        filename << "/home/ErHa/GANNS_Res/subdata/subData" << std::setw(4) << std::setfill('0') << i << ".bin";
        ...
        for(int l = 0; l < index_[i].size(); l++){
            ...
            for(int start = num_of_subgraph_points[i][l]; start < end; start++){
                int id = index_[i][l][start];
                file.write(reinterpret_cast<char*>(&dim_), sizeof(int));
                file.write(reinterpret_cast<char*>(sub_data + id * dim_), sizeof(char) * dim_);
                index_[i][l][start] = id + numBatchPoints * outIteration;
                new_index_of_points[i].push_back(index_[i][l][start]);
                // printf("0 in %d subData\n", i);
                // if(index_[i][l][start] == 0){
                //     for(int k =0; k < 128; k++){
                //         printf("%.0f ", float(sub_data[k]));
                //     }
                //     printf("\n");
                // }
            }
...
void RVQ::saveSubgraphIndex(int numPoints, std::vector<std::vector<int>> &new_index_of_points){
    printf("Save subgraph index\n");
    int* sizeSubgraph = new int[numCoarseCentroid_]();
    int* preFixSizeSubgraph = new int[numCoarseCentroid_ + 1]();
    ...
    for(int i = 0; i < index_.size(); i++){
        file2.write(reinterpret_cast<char*>(new_index_of_points[i].data()), sizeof(int) * new_index_of_points[i].size());
        for(int l = 0; l < index_[i].size(); l++){
            sizeSubgraph[i] += index_[i][l].size();
        }
        preFixSizeSubgraph[i + 1] = preFixSizeSubgraph[i] + sizeSubgraph[i];
    }
    file2.close();
    printf("There are %d points in total\n",preFixSizeSubgraph[numCoarseCentroid_]);
    std::ofstream file1("/home/ErHa/GANNS_Res/subdata/pre_fix_of_subgraph_size.bin", std::ios::binary);
    file1.write(reinterpret_cast<char*>(&numCoarseCentroid_), sizeof(int));
    file1.write(reinterpret_cast<char*>(preFixSizeSubgraph), sizeof(int) * (numCoarseCentroid_ + 1));
    file1.close();
}
