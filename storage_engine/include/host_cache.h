...
{   
    ...
    // @JIE-vital：local结构存储了从GPU驱逐页面的元数据，复制到gpu_rw_queue中的一个条目
...
void HostCache::handleRequest(int queue_index)
{
    ...
    if (type == GPU_RW_EVICT_TO_HOST) {
        ...
        // @JIE-vital:追踪主机缓存中可以用的槽，如果等待的请求数目超过256则返回失败的标志
        ...
        // 轮询主机缓存，直到找到一个空闲槽_bid，如果当前插槽被锁定或包含脏页，则继续查找下一个。
        do {
