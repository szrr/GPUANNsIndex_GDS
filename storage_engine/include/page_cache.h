...
//        printf("Cache Extra Reads: %llu\n", v);
    ...
    void add_range(range_t<T>* range) {
        ...
        // @JIE:h_ranges是一个结构体数组，用来存放range的状态
        // 每个range内有一个pages_t pages，相当于把它们统一给h_range管理了
...
uint32_t cudaDevice, data_dist_t dist) {
    ...
    rdt.read_io_cnt = 0;
    rdt.index_start = is;
    rdt.count = count;                      // 这里的count是包含的元素的数目，比如3个item，每个item里面128个向量和128个邻居的话，count=3*(128+128)
    //range_id = (c_h->range_count)++;
    rdt.page_start = ps;
    rdt.page_count = pc;                    // 整个图是一个range，这个指的是整个图可以被分成的页数
    rdt.page_size = c_h->pdt.page_size;     // 单个页的大小
...
uint64_t range_d_t<T>::get_global_address(const size_t page) const {
    // @JIE：用get_page得到索引i在某个range内的页号（不是全局的），然后把它和range id拼接在一起，就是一个全局id了
    // printf("global addr: %llu\n",(page << cache.n_ranges_bits) | range_id);
...
uint64_t range_d_t<T>::get_cache_page_addr(const uint32_t page_trans) const {
    // printf("((uint64_t)((cache.base_addr+(page_trans * cache.page_size)))) = %d\n",((uint64_t)((cache.base_addr+(page_trans * cache.page_size)))));
...
__forceinline__
__device__
// @JIE：传入的是range内的page id，数目（memcpy函数中，count设置为1）
...
{
    // @JIE：最底层的读写是以range为单位发出的，这也是分多个range可以增大并发性的原因
    // 涉及到大量的原子操作，如果同一个range发出，可能会存在大量的阻塞即时间浪费
    uint64_t index = pg;
    uint64_t b_page = get_backing_page(index);
    Controller* c = cache.d_ctrls[ctrl];
    read_data(&cache, (c->d_qps)+queue, ((b_page)*cache.n_blocks_per_page), cache.n_blocks_per_page, page_trans);
...
//                //printf("++tid: %llu\tbase: %p  page:%llu\n", (unsigned long long) threadIdx.x, base_master, (unsigned long long) page);
    ...
    int64_t find_range(const size_t i) const {
        int64_t range = -1;
        int64_t k = 0;
        // printf("i = %llu, n_ranges = %llu\n",i,n_ranges);
        for (; k < n_ranges; k++) {
            // printf("find range %ld [%lu - %lu] for index %ld\n", k, d_ranges[k].index_start, d_ranges[k].count + d_ranges[k].index_start, i);
            if ((d_ranges[k].index_start <= i) && (d_ranges[k].count + d_ranges[k].index_start > i)) {
                range = k;
                break;
            }
    ...
    void coalesce_page(const uint32_t lane, const uint32_t mask, const int64_t r, const uint64_t page, const uint64_t gaddr, const bool write,
        ...
        uint32_t active_cnt = __popc(mask);     //用于计算输入掩码 (mask) 中设置位（即值为1的位）的数量
                                                //mask 是一个位掩码，其中每一位对应一个线程，表示该线程是否活跃

        eq_mask = __match_any_sync(mask, gaddr);    //检查mask标识的线程中，哪些线程的 gaddr 值相同，并返回一个位掩码
        eq_mask &= __match_any_sync(mask, (uint64_t)this);  //将两个掩码进行按位与操作，结果表示在 mask 标识的活跃线程中，
                                                            //哪些线程同时具有相同的 gaddr 和 this 值
        master = __ffs(eq_mask) - 1;

        uint32_t dirty = __any_sync(eq_mask, write);//如果有任意一个线程在执行写操作，会返回非零值
...
//#endif
    ...
    void* acquire_page_(const size_t i, data_page_t*& page_, size_t& start, size_t& end, range_d_t<T>* r_, const size_t page) const {
        ...
        if (r_) {
            ...
            // @JIE:base_master是一缓存槽的id，根据它得到缓存槽的地址
    ...
    __forceinline__
    __device__
    T vec_access(const size_t i, const size_t k, const size_t len_item) const {
        size_t real_idx = i*len_item+k;
        uint32_t lane = lane_id();
        int64_t r = find_range(real_idx);
        auto r_ = d_ranges+r;
        T ret;
        ...
        if (r != -1) {
            ...
            uint64_t base_master;
            uint32_t count;
            uint64_t page = r_->get_page(i*len_item+k);
            uint64_t subindex = r_->get_subindex(i*len_item+k);
            uint64_t gaddr = r_->get_global_address(page);
    ...
    __forceinline__
    __device__
    T adjL_access(const size_t i, const size_t k, const uint32_t dim, const uint32_t len_item) const {
        // 可把邻居位的最后一位（len_item - 1）用来标识有效邻居的长度
        uint64_t item_start_idx = i*256;
        uint32_t lane = lane_id();
        int64_t r = find_range(item_start_idx);
        auto r_ = d_ranges+r;
        T ret;
        if (r != -1) {
#ifndef __CUDACC__
            ...
            uint64_t base_master;
            uint32_t count;
            uint64_t page = r_->get_page(item_start_idx);
            uint64_t subindex = r_->get_subindex(item_start_idx);
            uint64_t gaddr = r_->get_global_address(page);

            coalesce_page(lane, mask, r, page, gaddr, false, eq_mask, master, count, base_master);

            ret = ((T*)(r_->get_cache_page_addr(base_master)+subindex))[dim+k-1];
            __syncwarp(eq_mask);
            if (master == lane)
                r_->release_page(page, count);
            __syncwarp(mask);
    ...
    void seq_write(const size_t i, const T val) const {
        ...
        if (r != -1) {
            ...
            uint64_t base_master;
            uint32_t count;
            uint64_t page = r_->get_page(i);                // 索引i处的元素在当前range内的页号
            uint64_t subindex = r_->get_subindex(i);        // 索引i处的元素在它所属的页内的偏移
            uint64_t gaddr = r_->get_global_address(page); 
    ...
    __forceinline__
    __device__
    void item_write(const size_t i, const _Float32* item, const uint32_t len_item) const {
        // printf("Calling item_write \n");
        uint64_t item_start_idx = i*256;
        uint32_t lane = lane_id();
        int64_t r = find_range(item_start_idx);
        auto r_ = d_ranges+r;
        ...
        if (r != -1) {
            ...
            // printf("len_item = %d\n",len_item);
            for(;k<len_item;++k){
                // printf("Calling get_cache_page_addr\n");
                // @JIE:待优化
                ((T*)(r_->get_cache_page_addr(base_master)+subindex))[k] = item[k];
                // printf("%f\n",((T*)(r_->get_cache_page_addr(base_master)+subindex))[k]);
            }
            // printf("val = %d\n",val);
            __syncwarp(eq_mask);
            if (master == lane){
...
__forceinline__
__device__
// @JIE：接受的参数是range内的page index，range id
uint32_t page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t* bid) {
    ...
    do {
        ...
        if ( v == FREE ) {
            // @JIE：比较并交换：给v赋值LOCKED，如果成功，返回true
            ...
            if ( lock ) {
                ...
                // 前面上锁，然后修改page_translation，然后再释放锁
        ...
        // @JIE：该缓存页当前已经分配，但是没有被任何线程锁定，此时会被锁定然后处理该页的内容（驱逐或重用等）
        else if ( v == UNLOCKED ) {
            lock = this->cache_pages[page].page_take_lock.compare_exchange_weak(v, LOCKED, simt::memory_order_acquire, simt::memory_order_relaxed);
            if (lock) {
                            ...
                            if ((new_expected_state & DIRTY)) {
                                ...
                                if (ctrl == ALL_CTRLS) {
                                    ...
                                    //#if GET_PAGE_FAULT_RATE
                                        ...
                                        //else {
                                    #endif
                                            // printf("Calling write_data in case1:\n");
                                            write_data(this, (c->d_qps)+queue, (index*this->n_blocks_per_page), this->n_blocks_per_page, page);
                                            c->write_io_counter.fetch_add(1, simt::memory_order_relaxed);
                                    #if USE_HOST_CACHE && ENABLE_TMM
                                            #if PRINT_TMM
                                    ...
                                    uint32_t queue = queue_ % (c->n_qps);
                                    //index = ranges_page_starts[previous_range] + previous_address;
                                    // printf("Calling write_data in case2:\n");    //似乎从未进入过这个分支
                                    write_data(this, (c->d_qps)+queue, (index*this->    n_blocks_per_page), this->n_blocks_per_page, page);
                                    c->write_io_counter.fetch_add(1, simt::memory_order_relaxed);
                                }
                                evict_cnt->fetch_add(1, simt::memory_order_relaxed);
...
uint32_t page_cache_d_t::find_slot_prefetching(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t* bid) {
    ...
    do {
        // @JIE:page_ticket可以理解成一个公用的uint32变量，范围从0到最大cache页数减1，用来标记一个cache页
        // 先取得当前的值，再给他加1，这时候如果有其他的线程涉及到获取slot的操作，就不会影响到现在的页面
        page = page_ticket->fetch_add(1, simt::memory_order_relaxed)  % (this->n_pages);
        bool lock = false;
        uint32_t v = this->cache_pages[page].page_take_lock.load(simt::memory_order_relaxed);
